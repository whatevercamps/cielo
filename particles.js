...
var pJS = function (tag_id, params) {
  ...
  pJS.fn.particle = function (id, color, opacity, position) {
    /* size */
    this.id = id || 0;
    this.radius =
  ...
  pJS.fn.particlesCreate = function () {
    for (var i = 0; i < pJS.particles.number.value; i++) {
      pJS.particles.array.push(
        new pJS.fn.particle(i, pJS.particles.color, pJS.particles.opacity.value)
      );
    }
  };

  const randMsg = () => {
    while (partid == prevParti) {
      parti =
        pJS &&
        pJS.particles &&
        pJS.particles.array &&
        pJS.particles.array[
          Math.floor(Math.random() * pJS.particles.array.length)
        ];
      partid = parti.id;
    }
    prevParti = partid;
    ...
    const els = document.getElementsByClassName("msg");
    Array.prototype.forEach.call(els, (e) => e.remove());
    const msg = document.createElement("span");
    msg.className = "msg";
    const mind = Math.floor(Math.random() * mensajes.length);
    console.log("msg", { i: mind, msg: mensajes[mind] });
    msg.innerHTML = mensajes[mind];
    msg.style.position = "absolute";
    msg.style.left = `${parti.x + parti.radius}px`;
    msg.style.top = `${parti.y - parti.radius * 5}px`;
    document.getElementById("body").appendChild(msg);

    setTimeout(() => {
      const els = document.getElementsByClassName("msg");
      Array.prototype.forEach.call(els, (e) => {
        e.classList.add("disappear3");
        setTimeout(() => {
          parti.radius = pJS.particles.size.value;
          e.remove();
        }, 3000);
      });
    }, 2000);
  };
  setInterval(randMsg, 25000);
  ...
  pJS.fn.modes.pushParticles = function (nb, pos) {
    console.log("push", { nb: nb, pos: pos });
  ...
  pJS.fn.modes.repulseParticle = function (p) {
    if (
      ...
      if (
        ...
        if (currentEstrella != p.id) {
          p.radius = 10;
          console.log("pos", { x: x, y: y });
          console.log("pos es", { x: p.x, y: p.y });
          console.log("jeje", { prev: currentEstrella, new: p.id });
          const els = document.getElementsByClassName("msg");
          Array.prototype.forEach.call(els, (e) => e.remove());
          const msg = document.createElement("span");
          msg.className = "msg";
          ...
          msg.innerHTML = mensajes[mind];
          msg.style.position = "absolute";
          msg.style.left = `${p.x + p.radius}px`;
          msg.style.top = `${p.y - p.radius * 5}px`;
          msg.style.opacity = 0;
          document.getElementById("body").appendChild(msg);
          msg.style.opacity = 1;
          currentEstrella = p.id;
        }
      } else if (currentEstrella == p.id) {
        currentEstrella = null;
        console.log("entro al else");
        const els = document.getElementsByClassName("msg");
        Array.prototype.forEach.call(els, (e) => e.remove());
        p.radius = pJS.particles.size.value;
      }
  ...
  pJS.fn.vendors.eventsListeners = function () {
    ...
    if (pJS.interactivity.events.onclick.enable) {
      pJS.interactivity.el.addEventListener("click", function (event) {
        const x = event.pageX;
        const y = event.pageY;
        console.log("v", event);
        // Collision detection between clicked offset and element.
        pJS.particles.array.forEach(function (element) {
          if (
            ...
            const data = estrellas[element.id || 0];
            const modalTitle = document.getElementById("estrellaModalLabel");
